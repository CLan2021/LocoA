...
class Simulator:
    ...
    def format_data(self, win_size = 10, conv_size = 3):
        ...
        for txt in txts:
            meta = self.one_meta2[self.one_meta2.File_Name == os.path.splitext(txt)[0]]
            if len(meta) == 0:
                print(txt)
                continue
            ...
            monitor_cleaned = monitor_cleaned.iloc[self.ignored:,:]
            ...
            if self.group_func == 'mean':
                act_digest = pd.concat([monitor_cleaned_smooth.groupby(['h', 'mNcell']).mean().T, 
                                        monitor_cleaned_smooth.groupby(['h', 'mNcell']).std().T], axis=1)
            else:
